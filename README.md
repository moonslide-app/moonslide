...
-   **📝 Markdown-based:** Write your presentation in Markdown with useful syntax extensions.
...
-   **💻 Multi-Platform:** Install Moonslide on macOS, Windows or Linux.
...
1. **Simple**: Enjoy the convenience of the standard template, which provides you with everything you need to create stunning Markdown presentations.
2. **Advanced**: Create your own template and configure every aspect of your Reveal.js Presentation. You have control over basically everything! Use the standard template as a starting point.

### Export Possibilities

You can export your presentations as a Reveal.js HTML presentation or as a PDF.

If you choose to export to HTML, you can pick one of the following options:

-   **HTML Presentation Bundle**: Use this to export your presentation as a bundle that includes the HTML file of the presentation and all required assets, such as stylesheets, scripts and images. This allows you to move and share your presentation. _If you're unsure which to pick, this is probably the right one for you._
-   **HTML Presentation Only**: Use this to only export the HTML file of your presentation. Included files will be referenced from their original location, with a relative path from the export destination. Use this for advanced workflows, where you want to take care yourself on storing your assets.

## What is a Template? 🤔

A template is a folder which contains all relevant assets to generate the Reveal.js presentation from a Markdown file. It consists roughly of the following parts:

-   The used Reveal.js framework
...
-   The configuration of the application's toolbar.

The template can decide to present its functionality inside the different menus of the toolbar. With that, features are easily accessible for all users.
...
In the Markdown sections of the presentations, standard Markdown formatting options are supported. We use the [markdown-it](https://github.com/markdown-it/markdown-it) parser to parse the content. Additionally, two markdown extensions (Attributes and Bracketed Spans) are used to add the possibility to style individual elements.

### Slide and Slot Separators

The horizontal rule (`<hr>`) markers `---` and `***` have a special meaning when they are used at the start of a line. `---` marks the separation between a Markdown and a YAML block and splits the content up into individual slides. `***` separates individual _slots_ inside a slide. These are important when working with [layouts](#layout).
...
If an image is the only item inside a paragraph, it is transformed to a background-image of a `div`, so it fills out the container. The CSS classes `image` and `image-block` are applied, which can be styled inside the template.
...
If an image should fill the whole screen, you can add it inside the Front Matter Configuration following the keyword `background-image`. See [Reveal.js Data-Attributes](#revealjs-data-attributes) for more options.
...
In the Front Matter block of the first slide, there are some attributes which configure the whole presentation. You can also find these attributes inside the toolbar menu `Presentation`.
...
Every slide uses a layout to structure its contents in a certain way. Every layout has a number of _slots_, which are containers for your content. Use the slot separator `***` to fill content into the next slot. Take a look at the available [layouts](#layouts) of the standard template. You also find the available layouts inside the toolbar menu `+`, where you can directly insert a new slide with the chosen layout.
...
Custom classes can be added to every slide. These classes are usually defined inside the template. The standard template comes with a lot of useful classes to style your slides. You can find the available styles inside the toolbar menu `Slide Styles`. In some cases it may be more useful to only style a custom element using the Markdown [attributes syntax](#attributes).
...
You can specify every other keyword on the slide, and it will be directly passed to the HTML-Element (`<section>`) of the slide as a data-tag. In this way a lot of Reveal.js features can be used. Below are some possible values, which are available in Reveal.js. You can also find them inside the toolbar menu `Slide`.
...
There are two themes which you can choose from: `black` and `white`. Every theme includes its own CSS stylesheets which changes the appearance of the whole presentation.
...
-   **`Title`**: Groups the content into a title and a sub-layout. Available layouts are `title-content`, `title-cols-2`, `title-cols-3`, `title-grid-3`, `title-grid-3-right` and `title-grid-4`.
...
The standard template offers a range of utility CSS-classes. They are available in the toolbar menus `Style`, `Animation` and `Slide Styles`. Here is a list of with some utility classes (not nearly complete):
...
To define your own layout, just create an HTML-file with your layout and reference it inside `config.yml`. Put the token `@@slot@@` for every slot there is inside the layout. Take a look at the `cols-3.html` file, which defines the layout `cols-3` (with 3 slots).
...
The toolbar is useful to showcase the features your template offers. **The toolbar does not configure the behavior of the presentation in any way.** It just gives the user an easy way to interact with your template. The user can always use any attributes they like, even if they are not specified inside the toolbar.
...
-   **`animation`**: Works the same way as `styles` but contains only attributes connected to animations.
...
> In these examples, all keys start with a `.`. This is only the case because the values are inserted into Markdown using the [attribute syntax](#attributes). Inside `slideStyles.yml` where the values are inserted into Front Matter, you would use the class names without `.`.
